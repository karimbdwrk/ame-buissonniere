...
(function (factory) {
  ...
  var utils = function () {
    return {
      ...
      highlight: function highlight(suggestionValue, phrase) {
      ...
      debounce: function debounce(func, wait) {
        var timeout,
            debounceID = new Date().getUTCMilliseconds(); // First query in the chain
      ...
      mouseHoverDebounce: function mouseHoverDebounce(func, selector, wait) {
        var timeout;
        timeout = setTimeout(function () {
      ...
      getActiveInstance: function getActiveInstance() {
        ...
        if ($el.length > 0) {
          $el.each(function () {
            ...
            if (_typeof($input.data('autocomplete')) == 'object') {
  ...
  function DgwtWcasAutocompleteSearch(el, options) {
    ...
    that.isMouseDownOnSearchElements = false; // Initialize and set options:
  ...
  function _formatResult(suggestionValue, currentValue, highlight, options) {
    ...
    return suggestionValue.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&apos;').replace(/&lt;sup/g, '<sup').replace(/&lt;\/sup/g, '</sup').replace(/sup&gt;/g, 'sup>').replace(/&lt;(\/?(strong|b|br|span))&gt;/g, '<$1>').replace(/&lt;(strong|span)\s+class\s*=\s*&quot;([^&]+)&quot;&gt;/g, '<$1 class="$2">');
  }

  DgwtWcasAutocompleteSearch.prototype = {
    initialize: function initialize() {
      var that = this; // Remove autocomplete attribute to prevent native suggestions:
      ...
      that.initMobileMode();
      that.hideAfterClickOutsideListener(); // Mark as initialized
    ...
    createContainers: function createContainers(type) {
      var that = this,
          options = that.options; // Suggestions
      ...
      if ($('.' + options.containerClass).length == 0) {
        ...
        that.suggestionsContainer.addClass('woocommerce'); // Add conditional classes
    ...
    registerEventsSearchBar: function registerEventsSearchBar() {
      var that = this; // Click close icon
      ...
      that.el.closest('.' + that.options.formClass).on('submit.autocomplete', function (e) {
        ...
        if (that.suggestions.length > 0) {
          $.each(that.suggestions, function (i, suggestion) {
            if (typeof suggestion.type != 'undefined' && suggestion.type == 'product_variation') {
              that.select(i);
              e.preventDefault();
              return false;
            }
    ...
    registerEventsSuggestions: function registerEventsSuggestions() {
      var that = this,
          suggestionSelector = '.' + that.classes.suggestion,
          suggestionsContainer = that.getSuggestionsContainer(); // Register these events only once
      ...
      var alreadyClicked = false; // Redirect to the new URL after click a suggestions
    ...
    registerEventsDetailsPanel: function registerEventsDetailsPanel() {
      var that = this,
    ...
    registerIconHandler: function registerIconHandler() {
      var that = this,
      ...
      if ($('.js-dgwt-wcas-initialized').length == 0 && $('.js-dgwt-wcas-search-icon-handler').length > 0) {
        $(document).on('click.autocomplete', function (event) {
          if ($('.dgwt-wcas-layout-icon-open').length) {
            ...
            if (!($target.closest('.' + that.options.searchFormClass).length > 0 || $target.closest('.' + that.options.containerClass).length > 0 || $target.closest('.' + that.options.containerDetailsClass).length > 0)) {
    ...
    registerEventsFixedMenu: function registerEventsFixedMenu() {
      var that = this;
    ...
    initMobileMode: function initMobileMode() {
      var that = this;
      var $formWrapper = that.getFormWrapper();

      if ($formWrapper.hasClass('js-dgwt-wcas-mobile-overlay-enabled') && that.isMobileMode()) {
    ...
    applyFlexibleMode: function applyFlexibleMode() {
      var that = this;
    ...
    fixPositionDetailsBox: function fixPositionDetailsBox() {
      var that = this,
      ...
      $containerDetails.css(offset); // Not set position on the bigger search form
      ...
      var leftBorderCrossed = false; // Right border crossed
      ...
      cOffset = $containerDetails.offset(); // Left border crossed
    ...
    automaticAlignment: function automaticAlignment() {
      var that = this,
    ...
    getFormOffset: function getFormOffset() {
      var that = this,
          $form = that.getFormWrapper(),
          $suggestionsContainer = that.getSuggestionsContainer(); // Choose orientation
    ...
    getSuggestionsContainer: function getSuggestionsContainer() {
      var that = this;
      return $('.' + that.options.containerClass);
    },
    getDetailsContainer: function getDetailsContainer() {
      var that = this;
      return $('.' + that.options.containerDetailsClass);
    },
    scrollDownSuggestions: function scrollDownSuggestions() {
      var that = this,
    ...
    canShowDetailsBox: function canShowDetailsBox() {
      var that = this;
        ...
        utils.debounce(function () {
          ...
          that.currentRequest = $.ajax(ajaxSettings).done(function (data) {
            ...
            if (typeof result.suggestions !== 'undefined') {
              ...
              if (result.suggestions.length === 1 && typeof result.suggestions[0].type !== 'undefined' && result.suggestions[0].type === 'no-results') {
    ...
    getDetails: function getDetails(suggestion) {
      var that = this; // Disable details panel
      ...
      if (result != null) {
        ...
        $.each(that.suggestions, function (i, suggestion) {
          if (typeof suggestion.type != 'undefined' && suggestion.type != 'more_products' && suggestion.type != 'headline') {
            var itemData = {
        ...
        that.preloader('show', 'details', ''); // Prevent duplicate ajax requests
          ...
          success: function success(response) {
            ...
            if (typeof result.items != 'undefined') {
              for (var i = 0; i < result.items.length; i++) {
                ...
                that.cachedDetails[cacheKey] = {
                  html: result.items[i]['html']
                };
    ...
    updatePrices: function updatePrices(noAjax) {
      var that = this,
          i,
          j,
          productsToLoad = [];
      ...
      for (i = 0; i < that.suggestions.length; i++) {
        if (typeof that.suggestions[i].type != 'undefined' && (that.suggestions[i].type == 'product' || that.suggestions[i].type == 'product_variation')) {
              ...
              for (i = 0; i < response.data.length; i++) {
                ...
                if (that.suggestions.length > 0) {
                  for (j = 0; j < that.suggestions.length; j++) {
                    if (typeof that.suggestions[j].type != 'undefined' && (that.suggestions[j].type == 'product' || that.suggestions[j].type == 'product_variation') && that.suggestions[j].post_id == postID) {
    ...
    updatePrice: function updatePrice(index, price) {
      var that = this;
    ...
    applyCustomParams: function applyCustomParams(params) {
      var that = this; // Custom params (global)

      if (_typeof(dgwt_wcas.custom_params) == 'object') {
      ...
      if (_typeof(inputCustomParams) === 'object') {
    ...
    applyPreloaderForPrice: function applyPreloaderForPrice(index) {
      var that = this;
    ...
    prepareSuggestionObjectID: function prepareSuggestionObjectID(suggestion) {
      var objectID = '';
    ...
    detailsPanelSetScene: function detailsPanelSetScene(objectID) {
      var that = this,
          $containerDetails = that.getDetailsContainer(),
    ...
    detailsPanelAddToScene: function detailsPanelAddToScene(objectID) {
      var that = this,
          $containerDetails = that.getDetailsContainer(),
    ...
    detailsPanelClearScene: function detailsPanelClearScene() {
      var that = this,
          $containerDetails = that.getDetailsContainer(),
    ...
    selectFirstSuggestion: function selectFirstSuggestion(suggestions) {
      var that = this,
          index = 0,
          noResults = false;
      ...
      if (suggestions != 'undefined' && suggestions.length > 0) {
        $.each(that.suggestions, function (i, suggestion) {
          if (typeof suggestion.type != 'undefined' && suggestion.type != 'more_products' && suggestion.type != 'headline' && suggestion.type != 'no-results') {
            index = i;
            return false;
          }
    ...
    positionIconSearchMode: function positionIconSearchMode($formWrapper) {
      var that = this,
          side = 'right',
          formLeftValue = -20;
      var $form = $formWrapper.find('.' + that.options.formClass);
      ...
      var formLeftOffset = $form[0].getBoundingClientRect().left; // Is the icon on left or right side of screen?
      ...
      formLeftValue = Math.floor(-1 * (formWidth * iconLeftRatio)); // Prevent shifting to the left more than the icon position (also positioned from the left)

      formLeftValue = Math.max(formLeftValue, -1 * iconLeftOffset);
      $form.css({
        'left': formLeftValue + 'px'
      });
    },
    hideIconModeSearch: function hideIconModeSearch() {
    ...
    hideAfterClickOutsideListener: function hideAfterClickOutsideListener() {
      var that = this;
    ...
    suggest: function suggest() {
      ...
      $('body').removeClass('dgwt-wcas-nores'); // Build suggestions inner HTML:
        ...
        if (typeof suggestion.type == 'undefined' || suggestion.type != 'product' && suggestion.type != 'product_variation') {
          ...
          if (suggestion.taxonomy === 'product_cat') {
            ...
            if (typeof suggestion.breadcrumbs != 'undefined' && suggestion.breadcrumbs) {
              ...
              append += '<span class="dgwt-wcas-st-breadcrumbs">' + dgwt_wcas.labels["in"] + ' ' + suggestion.breadcrumbs + '</span>'; //@TODO RTL support
            }
          ...
          if (isImg) {
            html += '<span class="dgwt-wcas-si"><img src="' + suggestion.image_src + '" />TEST</span>';
          ...
          if (suggestion.type === 'vendor') {
            html += '<span class="dgwt-wcas-st-title">' + prepend + formatResult(suggestion.value, value, highlight, options) + append + '</span>'; // Vendor city
          ...
          var sugVarClass = suggestion.type === 'product_variation' ? ' dgwt-wcas-suggestion-product-var' : ''; // One suggestion HTML

          dataAttrs += typeof suggestion.post_id != 'undefined' ? 'data-post-id="' + suggestion.post_id + '" ' : '';
          dataAttrs += typeof suggestion.taxonomy != 'undefined' ? 'data-taxonomy="' + suggestion.taxonomy + '" ' : '';
          dataAttrs += typeof suggestion.term_id != 'undefined' ? 'data-term-id="' + suggestion.term_id + '" ' : '';
          html += '<a href="' + url + '" class="' + className + ' dgwt-wcas-suggestion-product' + sugVarClass + '" data-index="' + i + '" ' + dataAttrs + '>'; // Image

          if (is_img) {
            html += '<span class="dgwt-wcas-si">' + suggestion.thumb_html + '</span>';
          }

          html += is_img ? '<div class="dgwt-wcas-content-wrapp">' : ''; // Open Title wrapper

          html += '<div class="dgwt-wcas-st">'; // Custom content before title (3rd party)
          ...
          html += '<span class="dgwt-wcas-st-title">' + formatResult(suggestion.value, value, true, options) + parent + '</span>'; // Custom content after title (3rd party)
          ...
          var showPrice = options.showPrice === true && typeof suggestion.price != 'undefined',
              ...
              showMeta = showPrice || showMetaBefore || showMetaAfter; // @todo show sale and featured badges
          // Meta

          html += showMeta ? '<div class="dgwt-wcas-meta">' : ''; // Custom content before meta (3rd party)
      ...
      container.show(); // Add class on show

      $('body').addClass('dgwt-wcas-open'); // Reset the latest mousedown position
    ...
    adjustContainerWidth: function adjustContainerWidth() {
      ...
      realWidth = Math.round(parseFloat(realWidth.replace('px', ''))); // If width is auto, adjust width before displaying suggestions,
    ...
    positionPreloader: function positionPreloader($formWrapper) {
      var $submit = _typeof($formWrapper) == 'object' ? $formWrapper.find('.dgwt-wcas-search-submit') : $('.dgwt-wcas-search-submit');
     ...
     * @param cssClass
     */
    preloader: function preloader(action, place, cssClass) {
      var that = this,
          html,
          $container,
      ...
      var $preloader = $container.find('.' + defaultClass); // Action hide
    ...
    adjustScroll: function adjustScroll(index, direction) {
      var that = this;
      ...
      if (!that.options.preserveInput) {
        ...
        that.ignoreValueChange = true; //that.el.val(that.getValue(that.suggestions[index].value));
      }
    ...
    onSelect: function onSelect(index) {
      ...
      if (typeof suggestion.type != 'undefined') {
        if (suggestion.type === 'more_products' || that.actionTriggerSource === 'enter' && that.latestActivateSource != 'key' && suggestion.type != 'product_variation') {
    ...
    dispose: function dispose() {
      var that = this,
          ...
          $mobileHandler = $formWrapper.find('.js-dgwt-wcas-enable-mobile-form'); // Remove all events
      ...
      that.close(); // Remove mobile handler
    ...
    enableOverlayMobile: function enableOverlayMobile() {
      var that = this;
    ...
    disableOverlayMobile: function disableOverlayMobile($overlayWrap) {
      var that = this;
    ...
    elementOrParentIsFixed: function elementOrParentIsFixed($element) {
    ...
    gaEvent: function gaEvent(label, category) {
      var that = this;
  ...
  (function () {
    ...
    $(document).ready(function () {
      ...
      var mobileBreakpoint = dgwt_wcas.mobile_breakpoint; // Disable details panel on small screens
    ...
    $(window).on('load', function () {
      ...
      var interval = setInterval(function () {
        ...
        if (_typeof(activeEl) == 'object' && $(activeEl).length && $(activeEl).hasClass('dgwt-wcas-search-input')) {
    ...
    $(window).on('load', function () {
      setTimeout(function () {
        makeIDUnique();
        maybeReinit();
      }, 500); // Support for Elementor popups

      if (typeof window.elementorFrontend != 'undefined' && typeof window.elementorFrontend.documentsManager != 'undefined' && typeof window.elementorFrontend.documentsManager.documents != 'undefined') {
    ...
    function maybeReinit() {
      ...
      if ($inputs.length > 0) {
        $inputs.each(function () {
          if (_typeof($(this).data('autocomplete')) != 'object') {
